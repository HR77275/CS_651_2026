...
def _build_optimizer(param_groups, training_args):
    optimizer_name = training_args.optimizer.lower()
    weight_decay = training_args.optimizer_weight_decay
    eps = training_args.optimizer_eps

    if optimizer_name == "adam":
        return torch.optim.Adam(param_groups, lr=0.0, eps=eps, weight_decay=weight_decay)
    if optimizer_name == "adamw":
        return torch.optim.AdamW(param_groups, lr=0.0, eps=eps, weight_decay=weight_decay)
    if optimizer_name == "sgd":
        return torch.optim.SGD(
            param_groups,
            lr=0.0,
            momentum=training_args.optimizer_momentum,
    ...
    if optimizer_name == "lion":
        return LionOptimizer(
            param_groups,
            lr=0.0,
            betas=(training_args.lion_beta1, training_args.lion_beta2),
            weight_decay=weight_decay,
        )
    raise ValueError(
        f"Unknown optimizer '{training_args.optimizer}'. "
        "Choose one of: adam, adamw, sgd, lion."
    )


def _resize_optimizer_state(stored_state, old_tensor, new_tensor, mask=None):
    if stored_state is None:
        return None

    resized_state = {}
    old_rows = old_tensor.shape[0]
    for key, value in stored_state.items():
        if not torch.is_tensor(value):
            resized_state[key] = value
            continue
        if value.shape == old_tensor.shape:
            if mask is not None:
                resized_state[key] = value[mask]
            elif value.shape == new_tensor.shape:
                resized_state[key] = torch.zeros_like(new_tensor)
            else:
                extension = torch.zeros_like(new_tensor[old_rows:])
                resized_state[key] = torch.cat((value, extension), dim=0)
        elif value.ndim > 0 and value.shape[0] == old_rows:
            if mask is not None:
                resized_state[key] = value[mask]
            else:
                extension_shape = (new_tensor.shape[0] - old_rows,) + value.shape[1:]
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000001) #haolin change /10
    ...
    def training_setup(self, training_args, normal_lr=0.003, finetune=False):
        ...
        if finetune:
            ...
            ] #training_args.position_lr_init*5.5
        else:
            l = [
                ...
                {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"}  #normal * 0.01 previous *self.spatial_lr_scale*0.1
            ]
        self.optimizer = _build_optimizer(l, training_args)
        self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale,
                                                    lr_final=training_args.position_lr_final*self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.position_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps) #/100
    ...
    def reset_opacity(self):
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity, torch.ones_like(self.get_opacity)*0.02)) #0.01 10/22/2024#change to 0.005??
    ...
    def load_ply(self, path):
        ...
        if "opacity1" in plydata.elements[0]:
            ...
            opacities1 = np.asarray(plydata.elements[0]["opacity1"])#[..., np.newaxis]
            opacities2 = np.asarray(plydata.elements[0]["opacity2"])#[..., np.newaxis]
        else:
            normal = np.ones(np.shape(xyz))*0.6

            opacities1 = np.asarray(plydata.elements[0]["opacity"])#[..., np.newaxis]
            opacities2 = np.asarray(plydata.elements[0]["opacity"])#[..., np.newaxis]
    ...
    def replace_tensor_to_optimizer(self, tensor, name):
        ...
        for group in self.optimizer.param_groups:
            if group["name"] == name:
                ...
                stored_state = _resize_optimizer_state(stored_state, old_param, tensor)
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                stored_state = _resize_optimizer_state(stored_state, old_param, old_param[mask], mask=mask)
                del self.optimizer.state[old_param]
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                stored_state = _resize_optimizer_state(stored_state, old_param, new_tensor)
                del self.optimizer.state[old_param]
    ...
    def densification_postfix(self, new_xyz, new_normal, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
        "normal": new_normal,
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_normal, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation)
